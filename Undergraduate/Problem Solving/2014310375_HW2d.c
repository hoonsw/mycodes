...
#define COMPARE1(a, b)	(++(*cnt) && fmod(a*2017,10000) > fmod(b*2017,10000))
#define COMPARE2(a, b)	(++(*cnt) && fmod(a*2017,10000) >= fmod(b*2017,10000))
#define MAX_SIZE 1000000

/* Modify from here */
// You can add additional user-defined functions here
void InsertionSort(double* list, int n, int *cnt)
{
	int i,j,temp=0;
	printf("n : %d\n",n);
	for(i=1;i<n;i++)
...
void merge(double* list, int* cnt, int left, int center, int right)
{
	int i=left;
	int j=center+1;
	int cur=left;
...
void mergeSort(double* list, int* cnt, int left, int right)
{
	int center;
...
void Solution(double *list, int n, int *cnt) {
/* Modify from here */
...
int main() {
	...
	for (i = 0; i < n; i++) {
  	...
  	printf("\nDecrypted Data\n");
	for (i = 0; i < n; i++) {
		printf("%lf ", fmod(list[i] * 2017, 10000));
